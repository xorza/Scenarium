...
Graph background power-of-two scale wrapping now computes the exponent via `log2` (no loop), choosing the multiplier closest to 1.0 that keeps spacing in-range.
...
`StyleSettings` colors now serialize as `#RRGGBBAA` strings via per-field serde helpers (with backward-compatible
deserialization for old arrays).
`StyleSettings` now serializes `const_badge_offset` as a 2-element array via a serde helper while still accepting
`{ x, y }` maps on load.
...
Run control now uses the shared `ToggleButton` for an "autorun" switch; clicking toggles start/stop and keeps UI state in `GraphUi::autorun_enabled`.
...
`ActionUndoStack` now tracks a base offset for undo bytes so front trims advance a logical cursor and only compact when needed, avoiding frequent front-drains.
...
`common::serialize` now returns `Vec<u8>` and `deserialize` accepts bytes; text formats are UTF-8 bytes and `FileFormat::Bin` uses bincode.
Graph/func/execution/view serialize APIs now return bytes and deserialize from bytes; file load/save uses byte IO.
Graph tests now include a `FileFormat::Bin` roundtrip alongside YAML/JSON/Lua.
...
Added a junior-developer onboarding manual in `docs/onboarding.md` covering project overview, build/run, and workflow tips.
...
Node removal now captures incoming event subscriptions and `ViewGraph::remove_node` clears event subscribers for removed nodes; `ViewGraph::removal_payload` returns a `GraphUiAction::NodeRemoved` action.
Timers invoker now defines const IDs for frame events and run, using `FuncId::from_u128` literals to avoid runtime parsing.
ID newtypes now expose `from_u128` for const-friendly UUID initialization.
...
- Event-loop readiness now uses `tokio::sync::Barrier` instead of a custom ReadyState helper.
- `EventLoopCallback` now stores an optional inner callback and supports a `none()` constructor that results in a
  no-op `call()`.
