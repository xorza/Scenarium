...
mod file_cache {
    ...
    /// passthrough lambda skipped) *and* prunes its `input[0]` cone — the file-cache node is
    /// keyed on its path alone, so a reused one lets the executor cut its upstream; bypass
    /// forces a recompute despite the file, which re-needs that upstream.
    #[tokio::test]
    async fn present_serves_from_file_absent_writes_bypass_recomputes() {
        ...
        // Reopen (file present): the cache node is served from its file, and because it's
        // keyed on its path alone, the executor's cut prunes its `input[0]` cone — `get_a`
        // is *not* recomputed on reopen.
        ...
        assert_eq!(
            ...
            1,
            "the served file-cache node's input cone is cut — get_a stays unrun"
        );
        assert!(
            !stats.executed_nodes.iter().any(|n| n.node_id == get_a_id),
        ...
        // Bypass on (file still present): the cache node recomputes + overwrites anyway, which
        // re-needs its input — so get_a runs again.
