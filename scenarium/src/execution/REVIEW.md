...
- [x] **Compute total and per-node RAM usage in one cache traversal.** `resident_ram_stats` now returns `CacheRamStats { total, by_node }`, calls each resident value's accounting once per owner, deduplicates shared custom values only in the global total, and omits zero-byte nodes.

- [x] **Move hot-path logic assertions to debug builds.** Output-pool conversions, DFS coloring, reader increment/consumption, outcome transitions, cache arity, per-port release, and post-validation flatten invariants now use debug-only guards; counters and indexes use unchecked-cost release operations after those guards. Authored graph validation and disk/blob rejection remain release checks. Debug-only malformed fixtures verify index, counter, coverage, resident arity, and port invariants still trip.
