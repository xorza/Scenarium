...
- [ ] **Store coverage only where it is not derivable: the disk metadata states.** A resident `OutputSnapshot` stores both `values` and a `Vec<bool>` that is defined to equal `value != Unbound` (`scenarium/src/execution/cache/mod.rs:89`), then every invoke, stamp, take, and per-port clear must keep the two synchronized (`scenarium/src/execution/cache/mod.rs:124`, `scenarium/src/execution/cache/mod.rs:251`, `scenarium/src/execution/cache/mod.rs:398`). Keep coverage in `OnDisk` and the blob header because it enables a cheap no-body probe, but derive it from resident values when checking demand or crossing the disk boundary. This removes one allocation per resident node and an entire invariant; preserve disk-read validation by comparing decoded values with the header before admitting them. Verify sparse multi-output reuse, take-on-last-read, per-port clearing, hydration, and broader-coverage overwrite.
