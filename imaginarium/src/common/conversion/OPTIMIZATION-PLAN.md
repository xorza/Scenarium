...
## Phase 9: L_U8 Expansion & LA_U16↔F32 ✅ COMPLETED

Added missing SIMD paths for L_U8 → RGBA_U8/RGB_U8 and LA_U16 ↔ LA_F32.

### Benchmark Results (Scalar vs SIMD, 4096×4096)

| Conversion | Scalar | SIMD | Speedup |
|------------|--------|------|---------|
| L_U8 → RGBA_U8 | 9.68ms (1.73 Gelem/s) | 8.13ms (2.06 Gelem/s) | **+19%** |
| RGBA_U8 → L_U8 | 3.12ms (5.39 Gelem/s) | 2.46ms (6.81 Gelem/s) | **+27%** |
| RGB_U8 → L_U8 | 2.99ms (5.61 Gelem/s) | 1.99ms (8.41 Gelem/s) | **+50%** |
| LA_U16 → LA_F32 | Uses existing U16↔F32 generic path | +6-14% |
| LA_F32 → LA_U16 | Uses existing F32↔U16 generic path | +6-14% |

### Implementation Details

**L_U8 → RGBA_U8**:
- SSSE3: `_mm_shuffle_epi8` to broadcast L to R=G=B, OR with alpha mask (0xFF)
- AVX2: Same technique with 256-bit registers, processing 32 pixels per iteration
- NEON: `vst4q_u8` for interleaved store (R=G=B=L, A=255)

**L_U8 → RGB_U8**:
- SSSE3: `_mm_shuffle_epi8` to expand L L L pattern (16 pixels → 48 bytes)
- AVX2: Same technique with 256-bit registers
- NEON: `vst3q_u8` for interleaved store (R=G=B=L)

**LA_U16 ↔ LA_F32**:
- Reuses existing `convert_u16_to_f32_generic` and `convert_f32_to_u16_generic` with channels=2
- x86_64: AVX2 → SSE2 fallback
