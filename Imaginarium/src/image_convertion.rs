...
pub(crate) fn convert_image(from: &Image, to: &mut Image) -> anyhow::Result<()> {
    match (from.color_format.channel_size, to.color_format.channel_size) {
        // @formatter:off
        (ChannelSize:: _8bit, ChannelSize:: _8bit) =>
            match (from.color_format.channel_type, to.color_format.channel_type) {
          ...
          To: Copy + Pod + Bounded,
{
    ...
    assert_eq!(from.color_format.channel_size.byte_count(), size_of::<From>() as u32);
    assert_eq!(to.color_format.channel_size.byte_count(), size_of::<To>() as u32);

    if from.color_format.channel_count == to.color_format.channel_count
        && from.color_format.channel_size == to.color_format.channel_size {
        return;
    }

    let to_pixel_size = to.color_format.byte_count();
    let from_pixel_size = from.color_format.byte_count();


    let convert_pixel: ConvertFn<From, To> =
        match (to.color_format.channel_count, from.color_format.channel_count) {
    ...
    for i in 0..from.height {
        for j in 0..from.width {
            let from_offset = i * from.stride + j * from_pixel_size;
            let from_pixel: &[From] = bytemuck::cast_slice(
                &from.bytes[from_offset as usize..(from_offset + from_pixel_size) as usize]
            );

            let to_offset = i * to.stride + j * to_pixel_size;
