...
| **Synthetic flat / residual flat-fielding** | ★ | ◐ | Divide by a normalized background model to correct residual *multiplicative* vignetting/dust the calibration flat missed — **`background_extraction`'s `BackgroundMode::Divide` already does this** (`out = in / (model/mean)`, divisor floored). What's left is deriving a *dedicated* synthetic flat separately from the additive-gradient model. = StarTools *Wipe* (a by-product of gradient removal). |
