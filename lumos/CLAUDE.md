...
`stack(paths, config, progress, cancel)` (`stack.rs:79`, from disk) and `stack_images(frames, config, progress, cancel)` (`stack.rs`, in-memory) → `AstroImage`. The trailing `cancel: common::CancelToken` is a cooperative cancel token (also on `stack_cfa_master` and `calibrate_align_stack`; pass `CancelToken::never()` to opt out): it rides on `CacheCore` and is polled at every heavy loop — RAW-decode load (per frame), `calibrate_align_stack`'s per-frame closure (top + again before the heavy demosaic, so a frame cancelled mid-load skips its demosaic), star detection + registration/warp (per frame), the combine (`process_chunked{,_weighted}`, per row, since an in-memory stack is a single chunk), and defect-map detection (`DefectMap::detect_hot`/`detect_cold`, per pixel — it dominates a *cached*-master `build_masters`, so `from_images` now takes a `cancel` too). When set, the op bails early and returns `Error::Cancelled` (or, for `from_images`, a partial map the caller must turn into an error by re-checking the token). An in-flight single decode/demosaic isn't interruptible (external libraw / SIMD kernels), so up to ~`num_cores` frames finish their current step before a cancel takes hold. A `StackFrame { image, coverage: Option<Buffer2<f32>> }` bundles each in-memory frame with optional per-pixel coverage (e.g. from `warp`); plain `AstroImage`s convert via `.into()` (coverage `None` = fully covered).
