...
`drizzle_stack(Vec<DrizzleFrame<Path>>, config, progress)` (`stacking/drizzle/stack.rs`) → `StackProduct`: output `AstroImage` + normalized `coverage` `[0,1]`, channel-shaped absolute `weight` image (`Σwᵢ`, the geometric WHT), and `Some(linear_variance)` (`Σwᵢ²/(Σwᵢ)²`). Drizzle's quality channels are identical because its geometric weights are shared across channels. `drizzle_images` accepts the same coherent records with in-memory `AstroImage` sources.
