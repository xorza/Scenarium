...
- `frame_store/mod.rs` owns memory-budget arithmetic, RAM/mmap `StoredPlane`s, spill-directory cleanup, full-frame statistics for unregistered inputs, and the `StoredFrame`/`StoredLightFrame` records consumed by combine. Each `StoredLightFrame` owns its source statistics alongside channels and optional coverage/confidence planes. `combine/cache/loader/mod.rs` owns tier loading and cache sidecars; `combine/cache/mod.rs` owns the chunked combine engine; `combine/normalization/mod.rs` owns reference selection, temporary common-domain measurements, and affine fitting.
- Pipeline (`stack.rs`): `stack` (from paths → `LightCache`, quality planes `None`) / `stack_images` (in-memory `StackFrame`s → `LightCache`) / calibration (`CfaCache::from_paths`). Each: tier-select + load → optionally compute normalization → resolve weights → chunked per-pixel combine applying normalize → reject → accumulate. `LightCache` retains only final frame norms; common-domain measurements are discarded after fitting, and `Normalization::None` skips them. `run_stacking` (`CfaCache` → `CfaImage`) / `run_stacking_weighted` (`LightCache` → `StackProduct`). The latter accumulates each channel's effective `Σwᵢ` and `Σwᵢ²/(Σwᵢ)²` from the reducer's actual survivor indices; `StackProduct.weight`/`variance` are channel-shaped `AstroImage`s, while `coverage` remains one channel-independent geometric-support fraction. `align_and_stack` composes that product with an `AlignmentSummary` in `AlignStackResult`.
