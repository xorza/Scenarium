...
- `LinearImage` (`io/image/linear.rs`): `metadata: ImageMetadata` + `pixels: LinearPixels`; dimensions are derived from the owned planes.
- `LinearPixels` (`io/image/linear_pixels.rs`): `L(Buffer2<f32>)` or `Rgb([Buffer2<f32>; 3])` — **planar**, one buffer per channel. Imaginarium's `PlanarPixels<N,T>` exists only at the layout-conversion boundary so integer/RGBA states cannot enter `LinearImage`.
...
- Entry points: `LinearImage::from_file(path, context)` admits linear non-mosaic FITS and float TIFF, `CfaImage::from_file(path, context)` admits camera RAW and mosaic FITS, `CfaImage::save_fits` writes an exact checksummed sensor-domain master, and `PreviewImage::from_file(path, context)` owns the public `PREVIEW_IMAGE_EXTENSIONS` display policy. `LoadContext` carries cancellation, a decode-memory ceiling, and explicit FITS HDU/cube/checksum policy; strict defaults reject ambiguous HDUs and undeclared three-plane cubes. `LinearImage::from_pixels` and `from_planar_channels` build explicit in-memory scientific products. `mean()` uses parallel Kahan summation.
- `fits/` is split by ownership: `decode/mod.rs` orchestrates loads, `decode/selection.rs` owns HDU and checksum policy, `decode/plan.rs` owns header-first shape/memory preflight, `decode/pixels.rs` owns bounded reads and null validation, `metadata/` translates FITS headers, `cfa.rs` persists Lumos sensor-domain FITS, and `error.rs` maps `fits-well` failures.
...
- `fits` (`fits-well` I/O, header-first HDU selection and memory preflight, bounded row-section decode, checksum policy, `physical_f32()` BSCALE/BZERO scaling, null rejection, physical float preservation, ROWORDER/XBAYROFF flips, and checksummed CFA master read/write including Bayer/X-Trans metadata), `sensor` (`detect_sensor_type(filters, colors)` from libraw metadata), `error` (`ImageError`).
...
- `from_files` (`mod.rs`) stacks raw CFA frames through the full stacking pipeline (sigma-clipped mean at ≥8 frames, else median); `from_images` (`mod.rs`) derives hot defects from background-subtracted dark residuals, subtracts flat-dark/bias from the flat, detects cold defects from that unfloored response, then consumes it into a per-color normalized, clamped divisor. `save`/`load` persist this prepared bundle as a versioned, checksummed multi-extension FITS file (`fits.rs`: named float image HDUs plus a defect-map binary table), so reloads repeat neither defect detection nor flat preparation.
...
`StarDetector` holds reusable `DetectionResources` (pooled image buffers plus background mesh/interpolation workspaces); fallible `from_config` validates once through `StarDetectionConfigError`, then `detect(&LinearImage)` → `DetectionResult` (`stars: Vec<Star>` flux-sorted + `Diagnostics`). Measurement constructs `Star` directly, while filtering produces the `QualityFilterDiagnostics` component stored unchanged in `Diagnostics`. Alignment pipelines propagate invalid detection configuration through `AlignStackError` before doing useful work.
...
`drizzle_stack(Vec<DrizzleFrame<Path>>, config, context, progress)` (`stacking/drizzle/stack.rs`) → `StackProduct`: output `LinearImage` + normalized `coverage` `[0,1]`, shared absolute `weight` plane (`Σwᵢ`, the geometric WHT), and shared `Some(linear_variance)` (`Σwᵢ²/(Σwᵢ)²`). `drizzle_images` accepts the same coherent records with in-memory `LinearImage` sources.
