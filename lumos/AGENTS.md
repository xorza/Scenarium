...
- `fits` (`fits-well` I/O, `physical_f32()` BSCALE/BZERO scaling, null rejection, physical float preservation, ROWORDER/XBAYROFF flips, and checksummed CFA master read/write including Bayer/X-Trans metadata), `sensor` (`detect_sensor_type(filters, colors)` from libraw metadata), `error` (`ImageError`).
