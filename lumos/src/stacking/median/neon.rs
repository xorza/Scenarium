//! NEON SIMD optimizations for median calculation (aarch64).
//!
//! Uses SIMD min/max operations for efficient sorting network-based median.

use std::arch::aarch64::*;

/// Calculate median using NEON SIMD sorting network for small arrays.
///
/// # Safety
...
unsafe fn minmax(a: *mut f32, b: *mut f32) {
    unsafe {
        let va = vld1_dup_f32(a);
        let vb = vld1_dup_f32(b);
        let min = vmin_f32(va, vb);
        let max = vmax_f32(va, vb);
        *a = vget_lane_f32(min, 0);
        *b = vget_lane_f32(max, 0);
    }
}
