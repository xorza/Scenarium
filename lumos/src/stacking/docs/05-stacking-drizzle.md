...
| P0 | Output grid is fixed from one input size; there is no union/intersection/crop offset or WCS derivation. Finalization constructs `LinearImage::from_planar_channels`, which resets metadata. | Mosaics, transformed extents, units, astrometry, and provenance are lost. |
