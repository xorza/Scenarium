...
/// Apply 2D convolution using AVX2 intrinsics.
///
/// Processes 8 output pixels at a time for improved throughput.
/// Used for elliptical Gaussian kernels that are not separable.
///
/// # Safety
/// Caller must ensure AVX2 and FMA are available.
#[target_feature(enable = "avx2,fma")]
#[allow(dead_code)]
pub unsafe fn convolve_2d_avx2(
    input: &[f32],
    output: &mut [f32],
    width: usize,
    height: usize,
            ...
            while x + 8 <= width {
                ...
                for ky in 0..ksize {
                    ...
                    for kx in 0..ksize {
                        ...
                        if kval.abs() < 1e-10 {
                            continue; // Skip near-zero kernel values
                        }

                        let kv = _mm256_set1_ps(kval);

                        // For each of 8 output pixels, load shifted input
                        // output[x+i] needs input[x+i+kx-radius]
                        let base_sx = x as isize + kx as isize - radius as isize;

                        // Check if we can do a contiguous load
...
/// Apply 2D convolution using SSE4.1 intrinsics.
///
...
#[target_feature(enable = "sse4.1")]
#[allow(dead_code)]
pub unsafe fn convolve_2d_sse41(
    input: &[f32],
    output: &mut [f32],
    width: usize,
    height: usize,
