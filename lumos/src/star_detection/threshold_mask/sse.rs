...
/// SSE4.1 implementation for adaptive threshold mask with per-pixel sigma.
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "sse4.1")]
#[allow(unsafe_op_in_unsafe_fn)]
pub unsafe fn process_words_adaptive_sse(
    pixels: &[f32],
    bg: &[f32],
    noise: &[f32],
    adaptive_sigma: &[f32],
    ...
    let bg_ptr = bg.as_ptr();
    let noise_ptr = noise.as_ptr();
    let sigma_ptr = adaptive_sigma.as_ptr();
    ...
    for (word_idx, word) in words.iter_mut().enumerate() {
        ...
        if base_pixel + 64 <= total_pixels {
            ...
            for group in 0..16 {
                ...
                let bg_vec = _mm_loadu_ps(bg_ptr.add(px_idx));
                let noise_vec = _mm_loadu_ps(noise_ptr.add(px_idx));
                let sigma_vec = _mm_loadu_ps(sigma_ptr.add(px_idx));
