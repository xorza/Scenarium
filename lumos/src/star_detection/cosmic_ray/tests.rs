...
fn test_detect_cosmic_rays_sharp_peak() {
    ...
    pixels_data[center * size + center] = 1.0; // Sharp cosmic ray
    let pixels = Buffer2::new(size, size, pixels_data);

    let background = Buffer2::new(size, size, vec![0.1f32; size * size]);
    let noise = Buffer2::new(size, size, vec![0.01f32; size * size]);

    let config = LACosmicConfig::default();
    let result = detect_cosmic_rays(&pixels, &background, &noise, &config);
...
fn test_grow_mask() {
    ...
    let pixels = Buffer2::new(5, 5, vec![0.5f32; 25]); // All elevated
