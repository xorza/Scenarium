//! Tests for SIMD Gaussian computations.
...
fn scalar_jacobian(x: f32, y: f32, params: &[f32; 6]) -> [f32; 6] {
...
fn scalar_model(x: f32, y: f32, params: &[f32; 6]) -> f32 {
...
fn generate_test_data(n: usize, params: &[f32; 6]) -> (Vec<f32>, Vec<f32>, Vec<f32>) {
    ...
    for i in 0..n {
        ...
        data_x.push(x);
        data_y.push(y);
        data_z.push(scalar_model(x, y, params));
    }
...
mod scalar_tests {
    ...
    fn test_scalar_jacobian_matches_reference() {
        let params = [8.0f32, 8.0, 1.0, 2.5, 2.5, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(16, &params);
        ...
        for i in 0..16 {
            let j_ref = scalar_jacobian(data_x[i], data_y[i], &params);
            for j in 0..6 {
                assert!(
    ...
    fn test_scalar_chi2_matches_reference() {
        let params = [8.0f32, 8.0, 1.0, 2.5, 2.5, 0.1];
        let (data_x, data_y, mut data_z) = generate_test_data(16, &params);
        ...
        let chi2 = compute_chi2_scalar(&data_x, &data_y, &data_z, &params);

        // Compute reference
        let mut chi2_ref = 0.0f32;
        for i in 0..data_x.len() {
            let model = scalar_model(data_x[i], data_y[i], &params);
    ...
    fn test_scalar_single_pixel() {
        let params = [5.0f32, 5.0, 1.0, 2.0, 2.0, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(1, &params);
...
mod dispatch_tests {
    use super::super::{compute_chi2_simd, fill_jacobian_residuals_simd};
    use super::*;

    #[test]
    fn test_dispatch_jacobian_matches_scalar() {
        let params = [8.0f32, 8.0, 1.0, 2.5, 2.5, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(17, &params);
    ...
    fn test_dispatch_chi2_matches_scalar() {
        let params = [8.0f32, 8.0, 1.0, 2.5, 2.5, 0.1];
        let (data_x, data_y, mut data_z) = generate_test_data(17, &params);

        for (i, z) in data_z.iter_mut().enumerate() {
            *z += 0.01 * (i as f32);
        }

        let chi2_scalar = compute_chi2_scalar(&data_x, &data_y, &data_z, &params);
        let chi2_simd = compute_chi2_simd(&data_x, &data_y, &data_z, &params);
    ...
    fn test_dispatch_various_sizes() {
        ...
        for n in [
            ...
            let (data_x, data_y, data_z) = generate_test_data(n, &params);
...
mod avx2_tests {
    ...
    fn test_jacobian_residuals_matches_scalar() {
        ...
        for i in 0..16 {
            let j_scalar = scalar_jacobian(data_x[i], data_y[i], &params);
            let model = scalar_model(data_x[i], data_y[i], &params);
    ...
    fn test_chi2_matches_scalar() {
        ...
        let params = [8.5f32, 8.3, 0.9, 2.2, 2.2, 0.15];
        let (data_x, data_y, mut data_z) = generate_test_data(17, &params);
        ...
        let chi2_simd = unsafe { compute_chi2_avx2(&data_x, &data_y, &data_z, &params) };
    ...
    fn test_handles_small_arrays() {
        ...
        let params = [7.0f32, 7.0, 0.7, 2.0, 2.0, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(5, &params);
        ...
        let chi2 = unsafe { compute_chi2_avx2(&data_x, &data_y, &data_z, &params) };
    ...
    fn test_exactly_8_pixels() {
        ...
        let params = [4.0f32, 4.0, 1.0, 2.0, 2.0, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(8, &params);
    ...
    fn test_9_pixels_simd_plus_scalar() {
        ...
        let params = [5.0f32, 5.0, 0.8, 2.5, 2.5, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(9, &params);
        ...
        let j_scalar = scalar_jacobian(data_x[8], data_y[8], &params);
        for j in 0..6 {
            assert!(
    ...
    fn test_various_sigma_values() {
        ...
        for sigma in [0.5f32, 1.0, 2.0, 3.0, 5.0, 10.0] {
            let params = [8.0f32, 8.0, 1.0, sigma, sigma, 0.1];
            let (data_x, data_y, data_z) = generate_test_data(16, &params);
            ...
            assert!(
                chi2_simd < 1e-10,
                "sigma={}: chi2={} should be near zero",
                sigma,
                chi2_simd
            );
        }
    ...
    fn test_asymmetric_sigma() {
        ...
        let params = [8.0f32, 8.0, 1.0, 2.0, 3.5, 0.1]; // Different sigma_x and sigma_y
        let (data_x, data_y, data_z) = generate_test_data(16, &params);
        ...
        for i in 0..16 {
            ...
            for j in 0..6 {
                assert!(
                    approx_eq(jacobian[i][j], j_scalar[j], 1e-5, 1e-6),
                    "Asymmetric sigma: Jacobian[{}][{}] mismatch",
                    i,
                    j
                );
            }
    ...
    fn test_extreme_amplitude_values() {
        ...
        for amp in [0.001f32, 0.1, 1.0, 10.0, 1000.0, 65535.0] {
            let params = [8.0f32, 8.0, amp, 2.5, 2.5, 0.1];
            let (data_x, data_y, data_z) = generate_test_data(n, &params);
    ...
    fn test_pixel_at_center() {
        ...
        let data_z: Vec<f32> = (0..8)
            .map(|i| scalar_model(data_x[i], data_y[i], &params))
            .collect();
    ...
    fn test_all_results_finite() {
        ...
        let params = [12.0f32, 12.0, 5.0, 3.0, 3.0, 0.5];
        let (data_x, data_y, mut data_z) = generate_test_data(24, &params);
    ...
    fn test_buffer_reuse() {
        ...
        let (data_x1, data_y1, data_z1) = generate_test_data(16, &params);
        ...
        let (data_x2, data_y2, data_z2) = generate_test_data(8, &params);
        ...
        for i in 0..8 {
            let j_scalar = scalar_jacobian(data_x2[i], data_y2[i], &params);
            for j in 0..6 {
                assert!(
                    approx_eq(jacobian[i][j], j_scalar[j], 1e-5, 1e-6),
...
mod sse_tests {
    ...
    fn test_jacobian_residuals_matches_scalar() {
        ...
        let params = [8.0f32, 8.0, 1.0, 2.5, 2.5, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(12, &params);

        let mut jacobian_simd = Vec::new();
        let mut residuals_simd = Vec::new();
        unsafe {
            fill_jacobian_residuals_sse(
                &data_x,
                &data_y,
                &data_z,
                &params,
    ...
    fn test_chi2_matches_scalar() {
        ...
        let params = [8.5f32, 8.3, 0.9, 2.2, 2.2, 0.15];
        let (data_x, data_y, mut data_z) = generate_test_data(13, &params);

        for (i, z) in data_z.iter_mut().enumerate() {
            *z += 0.01 * (i as f32);
        }

        let chi2_simd = unsafe { compute_chi2_sse(&data_x, &data_y, &data_z, &params) };
    ...
    fn test_exactly_4_pixels() {
        ...
        let params = [4.0f32, 4.0, 1.0, 2.0, 2.0, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(4, &params);
    ...
    fn test_5_pixels_simd_plus_scalar() {
        ...
        let j_scalar = scalar_jacobian(data_x[4], data_y[4], &params);
        for j in 0..6 {
            assert!(
    ...
    fn test_handles_small_arrays() {
        ...
        let params = [7.0f32, 7.0, 0.7, 2.0, 2.0, 0.1];
        let (data_x, data_y, data_z) = generate_test_data(3, &params);
        ...
        let chi2 = unsafe { compute_chi2_sse(&data_x, &data_y, &data_z, &params) };
        assert!(chi2.is_finite());
    }
    ...
    fn test_various_sizes() {
        ...
        for n in [1, 2, 3, 4, 5, 7, 8, 9, 12, 15, 16, 17, 100] {
            ...
            let chi2_sse = unsafe { compute_chi2_sse(&data_x, &data_y, &data_z, &params) };
            let chi2_scalar = compute_chi2_scalar(&data_x, &data_y, &data_z, &params);
    ...
    fn test_all_results_finite() {
        ...
        let params = [12.0f32, 12.0, 5.0, 3.0, 3.0, 0.5];
        let (data_x, data_y, mut data_z) = generate_test_data(20, &params);
