//! Benchmarks for Gaussian SIMD vs Scalar comparison.
//!
//! Run with: `cargo test -p lumos --release bench_gaussian_simd -- --ignored --nocapture`
...
/// Generate test data for Gaussian profile fitting.
...
fn bench_jacobian_small(b: bench::Bencher) {
    ...
    let params = [8.5f32, 8.5, 1.0, 2.5, 2.5, 0.1];
    ...
    if simd::is_sse4_available() {
        b.bench_labeled("sse4", || {
            ...
            let mut residuals = Vec::new();
            unsafe {
                simd::sse::fill_jacobian_residuals_sse(
                    &data_x,
                    &data_y,
                    &data_z,
                    &params,
    ...
    if simd::is_avx2_available() {
        b.bench_labeled("avx2", || {
            ...
            let mut residuals = Vec::new();
            unsafe {
                simd::avx2::fill_jacobian_residuals_avx2(
                    &data_x,
                    &data_y,
                    &data_z,
                    &params,
    ...
    #[cfg(target_arch = "aarch64")]
    {
        b.bench_labeled("neon", || {
            ...
            let mut residuals = Vec::new();
            unsafe {
                simd::neon::fill_jacobian_residuals_neon(
                    &data_x,
                    &data_y,
                    &data_z,
                    &params,
...
fn bench_jacobian_medium(b: bench::Bencher) {
    ...
    let params = [12.5f32, 12.5, 1.0, 3.0, 3.0, 0.1];
...
fn bench_jacobian_large(b: bench::Bencher) {
    ...
    let params = [16.5f32, 16.5, 1.0, 4.0, 4.0, 0.1];
...
fn bench_chi2_small(b: bench::Bencher) {
    ...
    b.bench_labeled("scalar", || {
        let chi2 = simd::compute_chi2_scalar(&data_x, &data_y, &data_z, &params);
        black_box(chi2)
    });
    ...
    if simd::is_sse4_available() {
        b.bench_labeled("sse4", || {
            let chi2 = unsafe { simd::sse::compute_chi2_sse(&data_x, &data_y, &data_z, &params) };
            black_box(chi2)
        });
    }
    ...
    if simd::is_avx2_available() {
        b.bench_labeled("avx2", || {
            let chi2 = unsafe { simd::avx2::compute_chi2_avx2(&data_x, &data_y, &data_z, &params) };
            black_box(chi2)
        });
    }
    ...
    {
        b.bench_labeled("neon", || {
            let chi2 = unsafe { simd::neon::compute_chi2_neon(&data_x, &data_y, &data_z, &params) };
            black_box(chi2)
        });
    }
}
...
fn bench_batch_1000_jacobian(b: bench::Bencher) {
    ...
    let stars: Vec<_> = (0..n_stars)
        .map(|i| {
            ...
            generate_gaussian_data(stamp_size, cx, cy, 1.0, 2.5, 2.5, 0.1)
        })
        .collect();
    ...
    b.bench_labeled("scalar", || {
        ...
        let mut residuals = Vec::new();
        for (x, y, z) in &stars {
            simd::fill_jacobian_residuals_scalar(x, y, z, &params, &mut jacobian, &mut residuals);
        }
...
fn bench_batch_1000_chi2(b: bench::Bencher) {
    ...
    b.bench_labeled("scalar", || {
        ...
        for (x, y, z) in &stars {
            total += simd::compute_chi2_scalar(x, y, z, &params);
        }
        black_box(total)
    });
    ...
    if simd::is_sse4_available() {
        b.bench_labeled("sse4", || {
            let mut total = 0.0f32;
            for (x, y, z) in &stars {
                total += unsafe { simd::sse::compute_chi2_sse(x, y, z, &params) };
            }
            black_box(total)
        });
    }
    ...
    if simd::is_avx2_available() {
        b.bench_labeled("avx2", || {
            let mut total = 0.0f32;
            for (x, y, z) in &stars {
                total += unsafe { simd::avx2::compute_chi2_avx2(x, y, z, &params) };
            }
            black_box(total)
        });
    }
    ...
    #[cfg(target_arch = "aarch64")]
    {
        b.bench_labeled("neon", || {
            let mut total = 0.0f32;
            for (x, y, z) in &stars {
                total += unsafe { simd::neon::compute_chi2_neon(x, y, z, &params) };
            }
            black_box(total)
        });
    }
