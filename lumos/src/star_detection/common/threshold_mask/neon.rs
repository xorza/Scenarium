...
/// NEON implementation for adaptive threshold mask with per-pixel sigma.
#[cfg(target_arch = "aarch64")]
#[allow(unsafe_op_in_unsafe_fn)]
pub unsafe fn process_words_adaptive_neon(
    pixels: &[f32],
    bg: &[f32],
    noise: &[f32],
    adaptive_sigma: &[f32],
    ...
    for (word_idx, word) in words.iter_mut().enumerate() {
        ...
        if base_pixel + 64 <= total_pixels {
            ...
            for group in 0..16 {
                ...
                let bg_vec = vld1q_f32(bg_ptr.add(px_idx));
                let noise_vec = vld1q_f32(noise_ptr.add(px_idx));
                let sigma_vec = vld1q_f32(sigma_ptr.add(px_idx));
