...
/// SSE-accelerated threshold mask creation on a chunk.
///
...
pub unsafe fn process_chunk_sse<const INCLUDE_BACKGROUND: bool>(
    pixels: &[f32],
    bg: &[f32],
    noise: &[f32],
    sigma_threshold: f32,
    ...
    while i < unrolled_end {
        ...
        let (bg0, bg1, bg2, bg3) = if INCLUDE_BACKGROUND {
            (
                _mm_loadu_ps(bg_ptr.add(i)),
                _mm_loadu_ps(bg_ptr.add(i + 4)),
                _mm_loadu_ps(bg_ptr.add(i + 8)),
                _mm_loadu_ps(bg_ptr.add(i + 12)),
            )
        } else {
