...
impl PlateSolver {
    ...
    pub fn solve(
        ...
        let image_stars: Vec<DVec2> = image_stars
            .iter()
        ...
        let catalog_pixel_positions = self.catalog_to_approx_pixels(
            &catalog_stars,
            approx_ra,
            approx_dec,
            approx_scale,
        ...
        let hasher = QuadHasher::new()
            ...
            .with_max_quad_radius(image_size.x.max(image_size.y) as f64 * 0.5)
        ...
        let point_pairs: Vec<(DVec2, DVec2)> = correspondences
            .iter()
        ...
        let (ref_points, target_points): (Vec<DVec2>, Vec<DVec2>) =
            point_pairs.iter().copied().unzip();
        ...
        let matched_stars: Vec<PixelSkyMatch> = inlier_indices
            .iter()
            .filter_map(|&i| {
                ...
                if i < correspondences.len() {
                    ...
                    if img_idx < image_stars.len() && cat_idx < catalog_stars.len() {
                        ...
                        return Some(((pixel.x, pixel.y), (cat.ra, cat.dec)));
                    }
                }
                None
            })
    ...
    fn catalog_to_approx_pixels(
        ...
        let image_center = image_size.as_dvec2() / 2.0;
        ...
        catalog
            ...
            .map(|star| {
                ...
                let ra0_rad = center_ra.to_radians();
                let dec0_rad = center_dec.to_radians();
                ...
                // Convert to pixels
                image_center + DVec2::new(xi.to_degrees(), eta.to_degrees()) / scale_deg
            })
            .collect()
    }
    ...
    fn compute_wcs_from_transform(
        ...
        let crpix = image_size.as_dvec2() / 2.0;
        ...
        for &((px, py), (ra, dec)) in matched_stars {
            let pixel = DVec2::new(px, py);
            let dist = (pixel - crpix).length();
            if dist < 100.0 {
                // Stars near center
                sum_ra += ra;
                sum_dec += dec;
        ...
        let crval = if count > 0.0 {
            ...
            let closest = matched_stars
                .iter()
                .min_by(|a, b| {
                    let pa = DVec2::new(a.0.0, a.0.1);
                    let pb = DVec2::new(b.0.0, b.0.1);
                    let da = (pa - crpix).length_squared();
                    let db = (pb - crpix).length_squared();
        ...
        Wcs::new((crpix.x, crpix.y), crval, cd, (image_size.x, image_size.y))
    }
}
...
mod tests {
    ...
    fn create_synthetic_field() -> SyntheticField {
        ...
        let fov_deg = scale * image_size.x as f64 / 3600.0;

        for i in 0..10_i32 {
            for j in 0..10_i32 {
                let ra_offset = (i as f64 - 4.5) * fov_deg / 10.0;
                let dec_offset = (j as f64 - 4.5) * fov_deg / 10.0;
                let ra = center_ra + ra_offset / center_dec.to_radians().cos();
                let dec = center_dec + dec_offset;
        ...
        for star in &catalog_stars {
            let ra_diff = (star.ra - center_ra) * center_dec.to_radians().cos();
            let dec_diff = star.dec - center_dec;
            let x = 512.0 + ra_diff * 3600.0 / scale;
            let y = 512.0 + dec_diff * 3600.0 / scale;
            image_stars.push(DVec2::new(x, y));
        }
    ...
    fn test_plate_solver_with_preloaded_catalog() {
        ...
        match result {
            Ok(solution) => {
                ...
                let sep = angular_separation(ra, dec, field.center_ra, field.center_dec);
    ...
    fn test_not_enough_stars() {
        let image_stars = vec![DVec2::new(100.0, 100.0), DVec2::new(200.0, 200.0)]; // Only 2 stars

        let config = PlateSolverConfig::default();
        let solver = PlateSolver::new(config);

        let result = solver.solve(&image_stars, 180.0, 45.0, 1.0, UVec2::new(1024, 1024));
        assert!(matches!(
            result,
