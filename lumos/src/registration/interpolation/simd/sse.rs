...
/// Warp a row using AVX2 SIMD with Lanczos3 interpolation.
///
/// Processes 4 output pixels at a time. The speedup comes from:
/// 1. SIMD coordinate transformation (4 pixels at once)
/// 2. SIMD weight accumulation for each pixel
///
/// # Safety
/// - Caller must ensure AVX2 is available.
#[cfg(target_arch = "x86_64")]
...
pub unsafe fn warp_row_lanczos3_avx2(
    input: &[f32],
    input_width: usize,
    input_height: usize,
    output_row: &mut [f32],
    output_y: usize,
    inverse: &crate::registration::types::TransformMatrix,
    border_value: f32,
    normalize: bool,
    clamp: bool,
) {
    ...
    unsafe {
        ...
        let a_vec = _mm_set1_ps(ta);
        let d_vec = _mm_set1_ps(td);
        let g_vec = _mm_set1_ps(tg);
        let by_c_vec = _mm_set1_ps(by_c);
        let ey_f_vec = _mm_set1_ps(ey_f);
        let hy_1_vec = _mm_set1_ps(hy_1);
        ...
        for chunk in 0..chunks {
            ...
            _mm_storeu_ps(src_x_arr.as_mut_ptr(), src_x);
            _mm_storeu_ps(src_y_arr.as_mut_ptr(), src_y);
            ...
            for p in 0..4 {
                ...
                // Load 6x6 neighborhood and compute weighted sum using SIMD
                // Process 2 rows at a time using AVX (8 floats)
                let mut sum = _mm256_setzero_ps();
                let mut min_val = f32::MAX;
                let mut max_val = f32::MIN;

                // We process in groups of 6 pixels per row
                for j in 0..6 {
                    let py = y0 - 2 + j as i32;
                    let wyj = wy[j];

                    // Load 6 pixels (pad to 8 for AVX)
                    ...
                    // Load weights (pad to 8)
                    let mut weights = [0.0f32; 8];
                    for i in 0..6 {
                        weights[i] = wx[i] * wyj;
                    }

                    let pix_vec = _mm256_loadu_ps(pixels.as_ptr());
                    let wgt_vec = _mm256_loadu_ps(weights.as_ptr());

                    // FMA: sum += pixels * weights
                    sum = _mm256_fmadd_ps(pix_vec, wgt_vec, sum);
                }

                // Horizontal sum of the AVX register
                // sum = [a, b, c, d, e, f, g, h]
                // We need a + b + c + d + e + f (ignore g, h which are padding zeros)
                let mut sum_arr = [0.0f32; 8];
                _mm256_storeu_ps(sum_arr.as_mut_ptr(), sum);
                let mut pixel_sum: f32 = sum_arr[..6].iter().sum();
...
mod tests {
    ...
    #[test]
    #[cfg(target_arch = "x86_64")]
    fn test_avx2_lanczos3_warp_row() {
        if !cpu_features::has_avx2() {
            eprintln!("Skipping AVX2 Lanczos3 test - not available");
            return;
        }
        ...
        super::super::warp_row_lanczos3_scalar(
            &input,
            width,
            height,
            &mut output_scalar,
        ...
        for x in 0..width {
            assert!(
                (output_avx2[x] - output_scalar[x]).abs() < 1e-4,
                "x={}: AVX2 {} vs Scalar {}",
                x,
                output_avx2[x],
                output_scalar[x]
    ...
    #[test]
    #[cfg(target_arch = "x86_64")]
    fn test_avx2_lanczos3_with_clamp() {
        if !cpu_features::has_avx2() {
            eprintln!("Skipping AVX2 Lanczos3 clamp test - not available");
            return;
        }
        ...
        let height = 64;
        let input = create_test_image(width, height);
        let transform = TransformMatrix::similarity(5.0, 5.0, 0.1, 1.05);
        let inverse = transform.inverse();
