...
fn test_barrel_vs_pincushion_opposite_corrections() {
    ...
    let sip_barrel =
        SipPolynomial::fit_from_transform(&ref_b, &tgt_b, &transform, &config).unwrap();
    let sip_pincushion =
        SipPolynomial::fit_from_transform(&ref_p, &tgt_p, &transform, &config).unwrap();
...
fn test_different_k_values_produce_different_corrections() {
    ...
    let sip_1 = SipPolynomial::fit_from_transform(&ref_1, &tgt_1, &transform, &config).unwrap();
    let sip_2 = SipPolynomial::fit_from_transform(&ref_2, &tgt_2, &transform, &config).unwrap();
