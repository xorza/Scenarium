...
- [ ] Add `AstroImage::save(&self, path: impl AsRef<Path>) -> Result<(), Error>` method that handles conversion to imaginarium::Image internally
...
- [ ] Add `LiveStackConfig::builder() -> LiveStackConfigBuilder` with methods: `running_mean()`, `weighted_mean()`, `rolling_sigma_clip(window, sigma)`, `normalize(bool)`, `track_variance(bool)`, `build()`
