...
| `cancel_token.rs` | `CancelToken`: shared poll-only cooperative cancel flag (`Arc<AtomicBool>` wrapper). `cancel()`/`is_cancelled()`/`reset()`; clones share one flag. For cooperative bail-out in hot loops (`spawn_blocking`/rayon); `reset()` makes it reusable across operations. No async wait — use `tokio_util`'s token for that. Free fn `is_cancelled(&Option<CancelToken>)` for the common "optional token threaded through an op" check (`None` = not cancelled). |
...
- `Buffer2<T>` / `BitBuffer2` — 2D buffers. Both index by `(x, y)` tuple, linear `usize`, and (Buffer2) ranges. `BitBuffer2` has no `IndexMut` (bits aren't independently addressable); mutate via `set`/`set_xy`.
