...
- [ ] **Reject LZ4 values during encode when the paired decoder would reject them.** The encoder converts `temp_buffer.len()` with `as u32` and starts compression without applying any limit (`common/src/serde.rs:40-50`), while the decoder rejects uncompressed sizes above 1 GiB (`common/src/serde.rs:82-100`). A large value can therefore serialize successfully into output this crate refuses to read, and values above `u32::MAX` receive a wrapped header. Move the ceiling to a shared module constant, validate the JSON byte length before resizing/compressing, and use `u32::try_from` for the header. Exercise the length helper at the ceiling, ceiling plus one, and `u32::MAX + 1` without allocating multi-gigabyte fixtures; retain an ordinary LZ4 round trip to verify the wire format.
