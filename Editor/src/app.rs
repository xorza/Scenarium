            ...
            ui.vertical(|ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.button("Run").clicked() {
                        self.user_state
                            .worker
                            .run_once(self.user_state.graph_state.graph.clone());
                    }
                });
            });
        });
...
impl ScenariumApp {
    pub(crate) fn new(cc: &CreationContext) -> Self {
        ...
        let worker = Worker::new(
            move |logger| load_invokers(logger, functions_clone.lock().unwrap().as_mut()),
            move || egui_ctx.request_repaint(),
        );

        let functions = functions.lock().unwrap().clone();
        let function_templates = FunctionTemplates::from(functions.clone());
    ...
    fn save_yaml(&self, filename: &str) -> anyhow::Result<()> {
        serialization::save(&self.user_state.graph_state, &self.state, filename)
    }
    ...
    fn file_dialog(&mut self, ctx: &egui::Context) {
        ...
        if let Some((filename, dialog_type)) = dialog {
            ...
            match dialog_type {
                DialogType::OpenFile => self.load_yaml(filename.as_str()).unwrap_or_default(),
                DialogType::SaveFile => self.save_yaml(filename.as_str()).unwrap_or_default(),
        ...
        match user_event {
            AppResponse::ToggleNodeOutput(node_id) => {
                ...
                let node = self
                    ...
                    .graph_state
                    .graph
                    .node_by_id_mut(eng_node.node_id)
                    .unwrap();
    ...
    fn response(&mut self, node_response: eng::NodeResponse<AppResponse, EditorNode>) {
        ...
        match node_response {
            ...
            eng::NodeResponse::ConnectEventEnded {
                input: input_id,
                output: output_id,
            } => {
                let input_search = self
                    .user_state
                    .graph_state
                    .arg_mapping
                    .find_by_input_id(input_id);
                let output_search = self
                    .user_state
                    .graph_state
                    .arg_mapping
                ...
                match (input_search, output_search) {
                    ...
                    (
                        ...
                        let event_node = self
                            ...
                            .unwrap();
                        event_node.events[event_address.index as usize]
                            .subscribers
                            .push(node_id);
                    }
            ...
            eng::NodeResponse::SelectNode(node_id) => {
                ...
                let node = self
                    ...
                    .graph_state
                    .graph
                    .node_by_id_mut(eng_node.user_data.node_id)
                    .unwrap();
            ...
            eng::NodeResponse::DeleteNodeFull {
                node_id: _node_id,
                node,
            } => {
                self.user_state
                    .graph_state
                    .graph
                    .remove_node_by_id(node.user_data.node_id);
            }
            eng::NodeResponse::DisconnectEvent {
                input: input_id,
                output: output_id,
            } => {
                let input_search = self
                ...
                match (input_search, output_search) {
                    ...
                    (
                        ...
                        event
                            .subscribers
                            .retain(|subscriber| *subscriber != node_id);
                    }
                    _ => panic!("Invalid connection"),
                }
            }
            eng::NodeResponse::RaiseNode(_node_id) => {}
            eng::NodeResponse::MoveNode {
                node: _node_id,
                drag_delta: _delta,
            } => {}
        }
...
fn load_invokers(logger: Logger, out_funcs: &mut Vec<Function>) -> Vec<Box<dyn Invoker>> {
    ...
    vec![Box::new(basic_invoker), Box::new(timers_invoker)]
}
