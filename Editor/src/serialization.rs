...
use crate::eng_integration::{
    build_node_from_func, combobox_inputs_from_function, EditorNode, EditorState, register_node,
};
...
pub(crate) fn save(
    ...
    let positions: Positions = editor_state
        .graph
        .nodes
        .iter()
...
pub(crate) fn load(
    ...
    for node in graph_state.graph.nodes() {
        ...
        combobox_inputs.iter_mut().for_each(|combo_input| {
            let value = node.inputs[combo_input.input_index as usize]
                .const_value
                .as_ref();
        ...
        let eng_node_id = editor_state
            .graph
            .add_node(node.name.clone(), editor_node, |_, _| {});
        editor_state.node_order.push(eng_node_id);

        build_node_from_func(&mut editor_state.graph, function, eng_node_id);
        ...
        positions
            ...
            .map(|(_, (x, y))| {
                editor_state
                    .node_positions
                    .insert(eng_node_id, egui::Pos2 { x: *x, y: *y });
            });
    }
    ...
    graph_state
        ...
        .flat_map(|node| {
            node.inputs
                .iter()
                .enumerate()
                .map(|(index, input)| (node.id(), index, &input.binding))
        })
        .filter(|(_node_id, _index, binding)| binding.is_output_binding())
        .for_each(|(node_id, index, binding)| {
            ...
            let output_id = binding
                ...
                .map(|output_binding| {
                    graph_state
                        .arg_mapping
                        .find_output_id(&output_binding.output_node_id, output_binding.output_index)
                })
                .unwrap();
        ...
        .flat_map(|node| {
            ...
            node.events
                ...
                .flat_map(move |(event_index, event)| {
                    event.subscribers.iter().map(move |subscriber_node_id| {
                        (event_node_id, event_index, subscriber_node_id)
                    })
                })
        })
        .for_each(|(event_node_id, event_index, subscriber_node_id)| {
            let event_id = graph_state
                .arg_mapping
                .find_event_id(&event_node_id, event_index as u32);
            let trigger_id = graph_state.arg_mapping.find_trigger_id(subscriber_node_id);
