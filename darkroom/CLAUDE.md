...
- **`gui/node/`** — the node-body widget: `mod.rs` is `NodeUI` (node bodies +
  ...
  double-clicks), with sub-widgets `header` (title + `S`/`T`/`D`/`C`/`P`/`i`
  badges: subgraph / terminal / disable / cache / force-pure / inspect; the `C`
  chip toggles `Node::persist` Memory⇄Disk via `SetPersist`, and the `P` chip
  (shown only on impure, non-self-caching funcs) toggles `Node::force_pure` via
  `SetForcePure`), `port_row` (the two port
