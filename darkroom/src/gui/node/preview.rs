...
//! value pipeline — as it records, the node registers itself in the run-state
//! request registry (`RunState::watch`), so its output value is fetched on
//! demand — and the inspector's thumbnail renderer (`image_preview`).
//!
...
pub(crate) fn preview_section(ui: &mut Ui, rcx: RecordCtx<'_>, node: &SceneNode) {
    ...
    // the thumbnail fills in (only the visible/recorded preview nodes register).
    rcx.run_state.watch(node.id);
    let handle = rcx
        .run_state
        .values(node.id)
