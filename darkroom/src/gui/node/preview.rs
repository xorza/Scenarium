...
//! value pipeline — as it records, the node registers itself in the frame's
//! value-request registry (`ValueRequests::watch`), so its output value is
//! fetched on demand — and the inspector's thumbnail renderer (`image_preview`).
//!
