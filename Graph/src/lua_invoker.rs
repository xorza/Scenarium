...
impl LuaInvoker<'_> {
    ...
    fn substitute_functions(&self) {
        ...
        for lua_func_info in functions.iter() {
            let function_info_clone = lua_func_info.info.clone();
            let new_function = self.lua.create_function(
                move |_lua: &Lua, inputs: Variadic<Value>| -> Result<Variadic<Value>, Error>  {
                    ...
                    for (_i, _arg) in function_info_clone.outputs.iter().enumerate() {
            ...
            self.lua.globals().set(lua_func_info.info.name.clone(), new_function).unwrap();

            output_index += lua_func_info.info.outputs.len() as i64;
        }
    }
